...
## 자주 발생한 문제와 해결
- 다른 PC에서 열면 내 일정이 안 보일 때
  - 원인: 현재 앱은 서버 DB 없이 `localStorage`를 사용하므로, 기기마다 데이터가 분리 저장됨
  - 해결: 여러 기기에서 같은 데이터를 보려면 Firebase/Supabase 같은 백엔드 연동이 필요

- 다른 PC에서 "일정 추가" 버튼이 동작하지 않을 때
  - 원인: 일부 브라우저/환경(특히 비보안 HTTP)에서 `crypto.randomUUID()` 미지원
  - 해결: 현재 코드에 랜덤 ID fallback을 추가해 동작하도록 보완됨
